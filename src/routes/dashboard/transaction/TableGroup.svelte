<script lang="ts">
    ...
    if (invoice.id) {
      goto(`/dashboard/transaction/${invoice.id}`);
    }
  }
</script>
		...
		{#each invoices as invoice, i (i)}
			<Table.Row>
				{#each headings as heading, i}
					{#if page.url.href.includes('transaction')}
						<Table.Cell class="{heading.className} border-b border-dashed cursor-pointer border-gray-300 " onclick={() => handleRowClick(invoice)}>
